<template>
  <div>
    <div class="border flex items-center justify-between px-5 pt-5">
      <div>
        <h2 class="text-lg font-medium text-red-900">Websites</h2>
        <p class="text-sm text-gray-600">list of websites</p>
      </div>
      <!-- Add button starts here -->
      <button
        type="button"
        class="block bg-white hover:bg-gray-100 focus:bg-gray-100 text-gray-900 font-semibold rounded-md px-2 py-2 text-xs border border-gray-300"
        @click="isAdd = true"
      >
        <span
          ><PlusIcon class="flex w-5 h-5 mr-2 shrink-0 text-red-600" /> Add
        </span>
      </button>
      <!-- Add button starts here -->
    </div>
...
const authHeader: any = ref({
  ...
  Authorization:
    "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1IjoiNmIwNWMwMzI0Y2NhNGI2ZGI4ODc4MGExY2I2NmE4MDgiLCJkIjoiMTY4MDA3MCIsInIiOiJzYSIsInAiOiJmcmVlIiwiYSI6ImZpbmRlci5pbyIsImwiOiJ1czEiLCJleHAiOjE2ODMyNzE0MTd9.jsKsTJbG_yHWy0Sz45JWd7IoszW3X_BaERz5oHgRBjg",
});
